    ...
    window.setTimeout(function(){
      ...
      var int = window.setInterval(function(){
        if(UpdateStomp.inputState === 1){
          game.arrows.children.forEach(function(child, index){
            if(child.tint === 0xffffff){
              game.arr[index][cIndex] = 1;
            } else {
              game.arr[index][cIndex] = 0;
            }
          });
        } else {
          for(var i = 0; i < 4; i++){
      ...
      song.onStop.add(function(){
        clearInterval(int);
        document.body.innerHTML = game.arr;
      });
    }, 1000);
  },
  create: function(){
